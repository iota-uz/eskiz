/*
СМС шлюз от Eskiz.uz

Отправляйте СМС по всему миру, в любом количестве!  В тестовом статусе для отправки тестовых смс сообщений, Вы можете использовать только нижеуказанные тексты:  - Это тест от Eskiz      - Bu Eskiz dan test      - This is test from Eskiz

API version: 1.0.0
Contact: danil@iota.uz
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package eskizapi

import (
	"encoding/json"
)

// checks if the DispatchStatusResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &DispatchStatusResponse{}

// DispatchStatusResponse struct for DispatchStatusResponse
type DispatchStatusResponse struct {
	Status *string        `form:"status" json:"status,omitempty"`
	Data   []StatusCount  `form:"data" json:"data,omitempty"`
	Id     NullableString `form:"id" json:"id,omitempty"`
}

// NewDispatchStatusResponse instantiates a new DispatchStatusResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDispatchStatusResponse() *DispatchStatusResponse {
	this := DispatchStatusResponse{}
	return &this
}

// NewDispatchStatusResponseWithDefaults instantiates a new DispatchStatusResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDispatchStatusResponseWithDefaults() *DispatchStatusResponse {
	this := DispatchStatusResponse{}
	return &this
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *DispatchStatusResponse) GetStatus() string {
	if o == nil || IsNil(o.Status) {
		var ret string
		return ret
	}
	return *o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DispatchStatusResponse) GetStatusOk() (*string, bool) {
	if o == nil || IsNil(o.Status) {
		return nil, false
	}
	return o.Status, true
}

// HasStatus returns a boolean if a field has been set.
func (o *DispatchStatusResponse) HasStatus() bool {
	if o != nil && !IsNil(o.Status) {
		return true
	}

	return false
}

// SetStatus gets a reference to the given string and assigns it to the Status field.
func (o *DispatchStatusResponse) SetStatus(v string) {
	o.Status = &v
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *DispatchStatusResponse) GetData() []StatusCount {
	if o == nil || IsNil(o.Data) {
		var ret []StatusCount
		return ret
	}
	return o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DispatchStatusResponse) GetDataOk() ([]StatusCount, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *DispatchStatusResponse) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given []StatusCount and assigns it to the Data field.
func (o *DispatchStatusResponse) SetData(v []StatusCount) {
	o.Data = v
}

// GetId returns the Id field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *DispatchStatusResponse) GetId() string {
	if o == nil || IsNil(o.Id.Get()) {
		var ret string
		return ret
	}
	return *o.Id.Get()
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *DispatchStatusResponse) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Id.Get(), o.Id.IsSet()
}

// HasId returns a boolean if a field has been set.
func (o *DispatchStatusResponse) HasId() bool {
	if o != nil && o.Id.IsSet() {
		return true
	}

	return false
}

// SetId gets a reference to the given NullableString and assigns it to the Id field.
func (o *DispatchStatusResponse) SetId(v string) {
	o.Id.Set(&v)
}

// SetIdNil sets the value for Id to be an explicit nil
func (o *DispatchStatusResponse) SetIdNil() {
	o.Id.Set(nil)
}

// UnsetId ensures that no value is present for Id, not even an explicit nil
func (o *DispatchStatusResponse) UnsetId() {
	o.Id.Unset()
}

func (o DispatchStatusResponse) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o DispatchStatusResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Status) {
		toSerialize["status"] = o.Status
	}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	if o.Id.IsSet() {
		toSerialize["id"] = o.Id.Get()
	}
	return toSerialize, nil
}

type NullableDispatchStatusResponse struct {
	value *DispatchStatusResponse
	isSet bool
}

func (v NullableDispatchStatusResponse) Get() *DispatchStatusResponse {
	return v.value
}

func (v *NullableDispatchStatusResponse) Set(val *DispatchStatusResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableDispatchStatusResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableDispatchStatusResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDispatchStatusResponse(val *DispatchStatusResponse) *NullableDispatchStatusResponse {
	return &NullableDispatchStatusResponse{value: val, isSet: true}
}

func (v NullableDispatchStatusResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDispatchStatusResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
